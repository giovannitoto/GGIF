...
lposterior_function <- function(ind, out_MCMC, columns = "k") {
  ...
  if("W" %in% names(out_MCMC)) {
    mean_pr <- tcrossprod(out_MCMC$W, out_MCMC$mu[[ind]])
  } else {
  ...
  lp_sigma <- sum(dgamma(out_MCMC$sigmacol[[ind]], hyperpar$as, hyperpar$bs, log = TRUE))
  # log posterior
  lpost <- ll + lp_lambda + lp_beta + lp_mu + lp_b_mu + lp_sigma
  return(lpost)
}
